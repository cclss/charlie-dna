...
- 공개 API/인터페이스 수준에서 검증하라. 내부 구현 상세(private 함수, 내부 구조)에 의존하는 테스트를 쓰지 마라.
...
1. 기존 코드를 읽고, 구현을 작성하라.
2. 구현이 완성되면 테스트를 돌려라. 변경마다 테스트를 돌리지 마라 — 논리적 단위 완성 후 1회 실행하라.
3. 실패하면 구현을 수정하고 다시 돌려라.
4. grain 범위 안에서만 리팩터하라. 주변 코드를 리팩터하지 마라.

### 베이스라인 측정

구현 시작 전에 영향받는 패키지와 직접 의존 패키지(1-hop)의 테스트를 1회 실행하라. 이 시점에 이미 실패하는 테스트는 pre-existing failure다. 네 책임이 아니다 — `context/backlog.md`에 기록하고 pass 기준에서 제외하라. 네 변경으로 새로 실패한 테스트만 수정 대상이다.

### 테스트 수정 규칙

- **이 grain에서 작성한 테스트**: setup, teardown, import, 타입 맞춤 등 메커니즘 수정은 허용한다. 단, assertion의 의도(무엇을 검증하는지)를 변경하지 마라 — assertion 값 변경, 테스트 케이스 삭제, 검증 조건 완화는 금지다. 수정 시 인계 요약에 사유를 한 줄 기록하라.
- **기존 테스트 (이 grain 이전에 존재한 테스트)**: 수정하지 마라. 기존 테스트가 깨지면 네 구현이 잘못된 것이다. 구현을 수정하라.
- **기존 테스트가 진짜 틀린 경우** (오래된 동작을 테스트): 테스트를 고치고 결정을 `context/decisions/`에 기록하라.
...
- 기존 테스트가 진짜 틀린 거면 (오래된 동작을 테스트), 테스트를 고치고 결정을 `context/decisions/`에 기록하라.
