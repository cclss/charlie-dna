...
- **`context/masterplan.md`** — 필수. 프로젝트가 무엇이고, 왜 존재하고, 무엇을 목표로 하는지의 원천이다. 이 파일이 없거나 비어 있으면 신호하라 ("When to Signal" 참조).
- **`--context`** — 선택. 인간이 제공한 추가 레퍼런스 (기존 문서, 설계 스펙, 외부 자료). 전부 읽어라.
- **Existing codebase** — 기존 코드베이스가 있으면 탐색하라. `.x-ray/` 또는 동등한 것이 있으면 거기서 시작하라. 마스터플랜은 의도를 설명하고, 코드는 현실을 설명한다. 충돌 시 생성 파일에 차이를 기록하라.
